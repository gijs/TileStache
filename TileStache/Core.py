...
def _addRecentTile(layer, coord, auth, format, body, age=300):
    """ Add the body of a tile to _recent_tiles with a timeout.
    """
    key = (layer, coord, tuple(auth), format)
    due = time() + age
...
def _getRecentTile(layer, coord, auth, format):
...
class Layer:
    ...
    def getTileResponse(self, coord, extension, ignore_cached=False, auth=None):
        ...
        if not ignore_cached:
            ...
            try:
                body = cache.read(self, coord, auth, format)
            ...
            body = _getRecentTile(self, coord, auth, format)
        ...
        if body is None:
            try:
                ...
                if self.write_cache:
                    ...
                    cache.lock(self, lockCoord, auth, format)
                ...
                if body is None:
                    ...
                    try:
                        tile = self.render(coord, format, auth)
                        save = True
                    ...
                    if save:
                        cache.save(body, self, coord, auth, format)
                ...
                if lockCoord:
                    ...
                    cache.unlock(self, lockCoord, auth, format)
        
        _addRecentTile(self, coord, auth, format, body)
    ...
    def render(self, coord, format, auth):
        ...
        if self.doMetatile() or hasattr(provider, 'renderArea'):
            ...
            tile = provider.renderArea(width, height, srs, xmin, ymin, xmax, ymax, coord.zoom, auth)
        
        elif hasattr(provider, 'renderTile'):
            # draw a single tile
            width, height = self.dim, self.dim
            tile = provider.renderTile(width, height, srs, coord, auth)
